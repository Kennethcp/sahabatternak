...
const Input = () => {
  ...
  const addData = (newData) => {
    ...
    // Update totals based on the status of the new data
    if (newData.status === "OK") {
      setTotalAccepted(totalAccepted + parseInt(newData.jumlah, 10));
    } else if (newData.status === "Rejected") {
      setTotalRejected(totalRejected + parseInt(newData.jumlah, 10));
    }
  };
